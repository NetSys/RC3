...
private:
  ...
  static const uint16_t QS_TIMEINTERVAL = 150; //millis, taken from paper
  static const uint16_t QS_INTERVALS = 5; //taken from paper
  ...
  //Addded 9/16/2013 Justine Sherry for QuickStart
  uint32_t*    m_arrivalRate; 
  uint16_t      m_qsidx; //treat arrivalrate as circularbuffer
  double        m_lastUpdate;
  uint64_t      m_sinceLastUpdate;
  uint32_t      m_promised;
  uint32_t      m_newpromised;
