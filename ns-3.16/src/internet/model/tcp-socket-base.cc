...
//if(TraceMe) std::cout << Simulator::Now().GetSeconds() << " " << GetNode()->GetId() <<  " Received Packet!" << std::endl;
  ...
  if (0 == (tcpHeader.GetFlags () & TcpHeader::ACK))
    ...
    { // Case 2: Potentially a duplicated ACK
      ...
      if(tcpHeader.GetAckNumber () > m_nextTxSequence){
          //JUSTINE: FOR NOW, JUST RETURN AND PRETEND IT NEVER HAPPENED -- FIX WHEN WE IMPLE DOUBLETCP.
          return;
      }
...
void TcpSocketBase::SendSynAck(Ptr<Packet> p){
  SendEmptyPacket (TcpHeader::SYN | TcpHeader::ACK);
}
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if(TraceMe) std::cout << Simulator::Now().GetSeconds() << " " << GetNode()->GetId() << " TCP SEND " << seq << std::endl;
  ...
  //std::cout << "Sending packet... " << Simulator::Now().GetSeconds() << std::endl;
  if (m_endPoint)
    {
      m_tcp->SendPacket (p, header, m_endPoint->GetLocalAddress (),
                         m_endPoint->GetPeerAddress (), m_boundnetdevice);
...
TcpSocketBase::NewAck (SequenceNumber32 const& ack)
{
  ...
  NS_LOG_INFO ("TCP " << this << " NewAck " << ack <<
...
if(TraceMe) std::cout << Simulator::Now().GetSeconds() << " " << GetNode()->GetId() <<  " RTX " << m_txBuffer.HeadSequence ()  <<  std::endl;
  NS_LOG_FUNCTION (this);
